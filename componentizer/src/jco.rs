...
/// Componentize a TypeScript/JavaScript source file into a WASM component.
/// Requires `jco` to be installed: `npm install -g @bytecodealliance/jco`
/// TypeScript files are first transpiled with esbuild before passing to jco.
pub fn componentize(js_file: &Path, wit_file: &Path, output_wasm: &Path) -> Result<()> {
    // check if jco is installed
    let jco_check = Command::new("jco").arg("--version").output();
